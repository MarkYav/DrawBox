...
- **Fill Action Memory Optimization**: to handle memory overhead for massive color fills. Alternatives include (A) extracting a vector boundary outline to simplify with RDP (saves maximum memory but loses pixel-perfect edge precision), or (B) applying Span Consolidation / Run-Length Encoding to merge adjacent horizontal `FillSpan`s into rectangular blocks (retains 100% pixel precision with moderate memory savings).
